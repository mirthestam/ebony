...
using SearchCommand = Ebony.Backends.MPD.Connection.Commands.Search.SearchCommand;
...
public partial class Library
{
    ...
    {
        return await cache.GetOrAddAsync($"album:{albumId}", async () =>
        {
    ...
    {
        const string indexKey = "albums:all:index";
        
        var index = await cache.GetOrAddAsync(indexKey, async () =>
        {
            ...
            var albums = _mpdTagParser.ParseAlbums(allTags).ToList();

            var albumTasks = albums.Select(async a =>
            {
                await cache.GetOrAddAsync($"album:{a.Id}", () => Task.FromResult<AlbumInfo?>(a), cancellationToken);
                return $"album:{a.Id}";
            }).ToList();
            
            var keys = await Task.WhenAll(albumTasks);
            return new CollectionCacheEntry(keys, DateTimeOffset.UtcNow);
        }, cancellationToken);

        if (index == null)
        {
            return Enumerable.Empty<AlbumInfo>();
        }
        
        var albumKeys = index.ItemKeys;
        
        var albumTasksLoad = albumKeys
            .Select(key => cache.GetOrAddAsync(key, () => Task.FromResult<AlbumInfo?>(null), cancellationToken))
            .ToList();
        
        var albumResults = await Task.WhenAll(albumTasksLoad);
        return albumResults.Where(a => a != null).Cast<AlbumInfo>().ToList();
    }
    
    public override async Task<IEnumerable<AlbumInfo>> GetAlbumsAsync(Id artistId,
        CancellationToken cancellationToken = default)
    {
        var indexKey = $"albums:artist:{artistId}:index";
    ...
    {
        return await cache.GetOrAddAsync($"track:{trackId}", async () =>
        {
    ...
    protected override async Task<Stream?> GetAlbumResourceFromBackend(Id resourceId, CancellationToken token)
    {
        if (resourceId == Id.Empty)
        {
