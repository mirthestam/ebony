...
public partial class SearchPagePresenter(ILogger<SearchPagePresenter> logger, IEbony Ebony)
{
    ...
    private async Task Search(string searchTerm)
    {
        ...
        try
        {
            _searchResults = await Ebony.Library.SearchAsync(searchTerm, token);
