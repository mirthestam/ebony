...
public partial class ArtistsPagePresenter : IRecipient<LibraryUpdatedMessage>
{
    ...
    public ArtistsPagePresenter(ILogger<ArtistsPagePresenter> logger, IMessenger messenger, IEbony Ebony)
    {
        _logger = logger;
    ...
    private async Task RefreshArtistsAsync(CancellationToken externalCancellationToken = default)
    {
        ...
        try
        {
            var artists = await _ebony.Library.GetArtistsAsync(cancellationToken).ConfigureAwait(false);
