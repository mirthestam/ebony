...
public class LibraryCache : ILibraryCache
{
    ...
    public LibraryCache(ILogger<LibraryCache> logger, ConnectionContext connectionContext, IEbonyControl ebonyControl)
    {
        var baseCacheDir =
            Environment.GetEnvironmentVariable("XDG_CACHE_HOME")
            ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".cache");
        
        _diskCachePath = Path.Combine(baseCacheDir, "Ebony", Sanitize(connectionContext.Profile.Id.ToString()), "library");
        _logger = logger;
        _maxMemoryCacheItems = 1000;
        _jsonContext = new CacheJsonContext(ebonyControl);
