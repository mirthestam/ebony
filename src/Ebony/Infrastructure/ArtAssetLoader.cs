...
using Ebony.Infrastructure.Palette;
using Gdk;
using Microsoft.Extensions.Logging;

namespace Ebony.Infrastructure;

public partial class ArtAssetLoader(ILogger<ArtAssetLoader> logger, IEbony Ebony)
{
    public async Task<Art?> LoadFromAssetAsync(Id assetId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await using var stream = await Ebony.Library.GetAlbumResourceStreamAsync(assetId, cancellationToken)
